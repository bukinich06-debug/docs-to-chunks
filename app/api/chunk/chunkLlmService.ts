...
- Пути из блоков ⟦img ...⟧ — скриншоты/рисунки (pict); пути из блоков ⟦icon ...⟧ — иконки (icon).
- Скриншоты (блоки ⟦img path="..."⟧) задают границы чанков: в поле "images" одного чанка ДОЛЖЕН быть не больше одного пути скриншота (pict).
- ОБЯЗАТЕЛЬНО: если при разметке чанка к нему относятся два или больше скриншотов — такой чанк недопустим; разбей текст на отдельные чанки, пока у каждого чанка будет не больше одного скриншота (pict).
- Перечисления в одном абзаце («элемент A…, элемент B…, элемент C…»), у каждого из которых свой блок ⟦img⟧, — это НЕ один чанк: каждый такой элемент — отдельный чанк со своим скриншотом.
- Иконки (⟦icon ...⟧) на границы чанков не влияют: в одном чанке может быть любое количество иконок.
...
7. Поле "images" у чанка:
   - Скриншоты (pict): пути из блоков ⟦img path="..."⟧. В одном чанке ДОЛЖЕН быть ровно один такой путь или ни одного. Два и более путей скриншота в одном чанке — недопустимо: раздели чанк на несколько, у каждого — свой скриншот и свой фрагмент текста.
   - Иконки (icon): пути из блоков ⟦icon path="..."⟧ — без ограничения по количеству в чанке.
   Если у чанка нет связанных изображений — "images": [].
...
export async function callLLMForJsonChunks(
  ...
  const systemInstruction = buildJsonChunksInstruction(Array.from(allowed));
